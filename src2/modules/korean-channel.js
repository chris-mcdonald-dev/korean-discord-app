import {mute} from './mute/permissions';
...
function koreanObserver(message, counter, client) {
    ...
    client.channels
        ...
        .then((channel) => {
            ...
            console.log(
                `There have been: ${counter[channel.name].count} English messages sent in the practice-korean channel.`
            );
...
function koreanChannelWarning(message, client) {
    client.channels
        ...
        .then((channel) => {
            message.reply(
                `Promise you won't hate me, but let's try to keep this room for practicing Korean!\nPlease use ${channel} for longer conversations in English. :smiley:`
            );
        })
        .catch(console.error);
}
