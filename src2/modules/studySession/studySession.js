...
import {replyInfo, replySuccess, replyError, replySurvey, sendDirectMessage, addMessageReaction} from '../../utils';
import {STUDY_SESSION} from '../../constants/studySession';
...
function createStudySession(message) {
    ...
    StudySession.create(studySession)
        .then(() => {
            ...
            addMessageReaction(message, null, ['⭐', '❌']);
        })
    ...
    if (studySession.subscribersId?.length > 0)
        return studySession.subscribersId.map((subscriberId) => {
            return message.client.users
                .fetch(subscriberId)
                .then((subscriber) =>
                    sendDirectMessage(subscriber, STUDY_SESSION.CANCEL.NOTIFICATION(message.author, subscriber))
                )
                .catch((error) => replyError(message, STUDY_SESSION.CANCEL.ERROR(error)));
        });
}
    ...
    StudySession.findOne({'message.id': message.id}, (error, studySession) => {
        ...
        studySession
            ...
            .then(() =>
                replySuccess(
                    message,
                    studySession.subscribersId.length > 0
