const {mute} = require('./mute/permissions');
import {logMessageDate} from '../utils/channels';
...
function resourcesObserver(message, counter, client) {
    ...
    counter[message.author.username + ' ' + message.author.id] =
        counter[message.author.username + ' ' + message.author.id] || {};

    counter[message.author.username + ' ' + message.author.id][message.channel.name] =
        counter[message.author.username + ' ' + message.author.id][message.channel.name] || {};

    counter[message.author.username + ' ' + message.author.id][message.channel.name].timeoutFlag =
    ...
    console.log(
        `${message.author.username}'s Message number: ${
            counter[message.author.username + ' ' + message.author.id][message.channel.name].count
        }`
    );
