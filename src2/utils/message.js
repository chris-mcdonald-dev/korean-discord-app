...
function messageTemplate(
    author = undefined,
    {title = '', content = '', description = '', color = '', url = '', fields = undefined}
) {
    return {
        content,
...
function replySurvey(message, survey, surveyOptions, delay) {
    return message.channel.send(survey).then(async (survey) => {
        addMessageReaction(survey, null, surveyOptions);
        return survey
            .awaitReactions(
                (reaction, user) => user.id === message.author.id && surveyOptions.includes(reaction.emoji.name),
                {max: 1, time: delay}
            )
...
export {replyInfo, replySuccess, replyError, replySurvey, sendDirectMessage};
